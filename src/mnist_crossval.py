...
def main(model='mlp', num_epochs=500,continuation_run=0,target_property='Ef',datapath=os.getcwd()+"/..",reg=0.0,aug=0.0):
    ...
    # Save the initial values for re-rounds
    initial_values = lasagne.layers.get_all_param_values(network)
    for cv in range(7):
        X_train, y_train, X_val, y_val, X_test, y_test = datahandle.load_ewald(target_property,datapath,cv)
        lasagne.layers.set_all_param_values(network, initial_values)
        ...
        for epoch in range(num_epochs):
            ...
            if epoch%isave==0 and cv==0:
                allparams.append([*lasagne.layers.get_all_param_values(network)])
                #activations.append([*lasagne.layers.get_output(network)])
            train_errors[epoch]=train_errors[epoch]+train_err/train_batches
            val_errors[epoch]=val_errors[epoch]+val_err/val_batches
            ###################################################################################
            
            # Then we print the results for this epoch:
            print("Epoch {} of {} of cross-val {} took {:.3f}s".format(
                epoch + 1, num_epochs, cv, time.time() - start_time))
        ...
        for batch in datahandle.iterate_minibatches(X_test, y_test, 50, shuffle=False):
            ...
            test_err += err
            test_batches += 1
        test_errors=test_errors + test_err / test_batches

    # After cross validation, we average the obtained errors
    train_errors=train_errors/7.0
    val_errors=val_errors/7.0
    test_errors=test_errors/7.0
    
    print("Final results:")
    print("  test loss:\t\t\t{:.6f}".format(test_errors))
